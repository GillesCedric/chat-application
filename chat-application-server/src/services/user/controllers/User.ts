...
export default class UserController {
  ...
  public readonly signIn = async (
    ...
    try {
      ...
      if (Crypto.decrypt(user.is2FAEnabled, "boolean") == "true") {
        ...
        validity.setMinutes(
          validity.getMinutes() +
            Number.parseInt(process.env.TWOFA_TEL_CODE_DELAY)
        );
          ...
          body: fs
            .readFileSync(
              path.join(
                ...
                "src",
                "templates",
                "2FATelAuthentication.txt"
              ),
              "utf8"
            )
            .replace("{{USERNAME}}", Crypto.decrypt(user.username, "username"))
            .replace("{{CODE}}", userCode)
            .replace("{{APPNAME}}", "Chat-Application")
            .replace("{{CODE_DELAY}}", process.env.TWOFA_TEL_CODE_DELAY),
        });
  ...
  public readonly activateEmail = async (
    req: Request,
    res: Response
  ): Promise<Response> => {
    try {
  ...
  public readonly activateTel = async (
    req: Request,
    res: Response
  ): Promise<Response> => {
    try {
      ...
      const tokenMatches = bcrypt.compareSync(req.body.code, token.token);
  ...
  public readonly verifyTel = async (
    req: Request,
    res: Response
  ): Promise<Response> => {
    try {
      ...
      validity.setMinutes(
        validity.getMinutes() +
          Number.parseInt(process.env.VERIFY_TEL_CODE_DELAY)
      );
      ...
      await Mailer.sendSMS({
        ...
        body: fs
          .readFileSync(
            path.join(process.cwd(), "src", "templates", "TelVerification.txt"),
            "utf8"
          )
          .replace("{{USERNAME}}", Crypto.decrypt(user.username, "username"))
          .replace("{{CODE}}", userCode)
          .replace("{{APPNAME}}", "Chat-Application")
          .replace("{{CODE_DELAY}}", process.env.VERIFY_TEL_CODE_DELAY),
      });
    ...
    try {
      const user = await UserModel.insertMany({
        ...
        picture: Crypto.encrypt(
          ...
          }/images/default/profile/${req.body.picture}`,
          "database"
        ),
      ...
      if (user) {
        ...
        await Mailer.sendMail({
          ...
          html: fs
            ...
            .replace(
              "{{CODE_DELAY}}",
              ((process.env.VERIFY_EMAIL_CODE_DELAY as unknown as number) /
  ...
  public readonly updateTokens = async (
    req: Request,
    res: Response
  ): Promise<Response> => {
  ...
  public readonly isValidTokens = async (
    req: Request,
    res: Response
  ): Promise<Response> => {
