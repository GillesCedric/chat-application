...
export default class Session {
  ...
  public static readonly authenticate = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
    ...
    } else if (((req.body.refresh_token && await JWTUtils.getUserFromToken(req.body.refresh_token, req.headers['user-agent'], Tokens.refreshToken) != undefined) || (req.query.refresh_token && await JWTUtils.getUserFromToken(req.query.refresh_token as string, req.headers['user-agent'], Tokens.refreshToken) != undefined)) && req.path.substring(req.path.lastIndexOf('/') + 1, req.path.length) == 'token') {
      return next()
    } else if (
